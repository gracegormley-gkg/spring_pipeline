"""
Stage 2 final fallback — embedding-similarity section detection.

Uses sentence-transformers/all-MiniLM-L6-v2 to find heading-like lines that
embed close to canonical descriptors of required sections. Only runs when
both the regex pass and the AI-TOC pass missed a section that downstream
stages need.

Imports sentence-transformers + numpy lazily so the module loads even when
the deps aren't installed; the caller (pipeline.sections.run) catches and
degrades to "not_found" without aborting the pipeline.

Ported verbatim from v1_multiagent_pipeline/pipeline/sections_embedding.py.
"""
...
def _candidate_headings(raw: str, max_lines: int = 5000) -> list[tuple[int, str]]:
    ...
    or title-case, not ending in a period. Returns (char_offset, line_text)."""
    ...
    while pos < n and len(out) < max_lines:
        ...
        if 4 <= len(line) <= 80:
            if line[0].isupper() and not line.endswith("."):
                upper_ratio = (
                    sum(1 for c in line if c.isupper())
                    / max(1, sum(1 for c in line if c.isalpha()))
                )
...
def embedding_detect(
    ...
    """For each missing section, score candidate headings against its
    descriptor; emit DetectedSection when (best ≥ threshold) AND (best -
    runner ≥ margin). If best clears threshold but margin fails, emit
    status='ambiguous' so Stage 4 can route to review."""
    descriptors = {
