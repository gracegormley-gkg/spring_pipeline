...
from pages import list_doc_ids, load_doc
...
def build_candidate_pool() -> list[dict]:
    """Build candidate records joining NUL works to local page JSON + cheap features."""
    works = fetch_collection_works()
    known = set(list_doc_ids())

    candidates: list[dict] = []
    for w in works:
        doc_id = find_ocr(w, known)
        if doc_id is None:
            continue
        try:
            doc = load_doc(doc_id)
            ...
            continue
        n_pages = doc.n_pages
        if n_pages < 1 or len(doc.full_text.strip()) < 200:
            continue
        ...
        head = doc.text_for_pages(1, 2)
        candidates.append({
            "work_id": w.get("id"),
            "doc_id": doc_id,
            "title": title,
            "estimated_pages": n_pages,  # now a true count
            "ocr_chars": len(doc.full_text),
            "eis_type_guess": cheap_eis_type(title, head),
            "bureau_guess": primary_bureau(contributors),
            "contributors": contributors,
        })
    log.info(f"Candidate pool: {len(candidates)} (NUL works with matched local pages)")
    return candidates
