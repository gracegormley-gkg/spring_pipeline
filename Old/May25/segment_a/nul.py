...
from config import COLLECTION_ID, NUL_API_BASE, NUL_CACHE_PATH
...
def find_ocr(work: dict, known_doc_ids: set[str]) -> Optional[str]:
    """Match a NUL work to a known doc_id (pages_data dir). Returns doc_id or None."""
    accession = work.get("accession_number", "")
    if accession:
        if accession in known_doc_ids:
            return accession
