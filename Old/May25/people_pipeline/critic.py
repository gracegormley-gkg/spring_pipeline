...
def critique_row(row: dict, doc: Doc) -> dict:
    """Returns the row with a `critic` block added (and `_critic_usage` for aggregation)."""
    cited_text = _resolve_cited_text(doc, row.get("evidence_pages") or [])
...
def critique_all(rows: list[dict], doc: Doc, parallel: int = settings.CRITIC_PARALLEL) -> list[dict]:
    """Run the per-row critic in parallel; preserves input order via sequence."""
    if not rows:
        return []
    out: list[dict] = []
    with ThreadPoolExecutor(max_workers=parallel) as pool:
        futures = {pool.submit(critique_row, r, doc): r for r in rows}
