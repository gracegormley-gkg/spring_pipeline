...
from nul import fetch_collection_works
from pages import Doc, load_doc
...
def process_doc(work: dict, doc_id: str, doc: Doc, force: bool = False) -> dict:
    ...
    critiqued = critique_all(merged, doc)
    ...
    output = {
        ...
        "schema": {
            ...
            "page_numbers": "EXACT — from per-page JSON source; verified pages override LLM-reported pages for verbatim-matched quotes.",
...
def cmd_process(args) -> int:
    ...
    if args.doc:
        ...
        work = next(
            (w for w in works_by_id.values() if find_ocr(w, {args.doc})),
            None,
        )
        if work is None:
            log.error(f"No NUL work matches {args.doc}")
            return 1
        process_doc(work, args.doc, doc, force=args.force)
        return 0
    ...
    for i, s in enumerate(todo, 1):
        ...
        doc = _load_doc_or_warn(s["doc_id"])
        if work is None or doc is None:
            log.warning("  Skipping: missing work or per-page JSON")
            continue
        try:
