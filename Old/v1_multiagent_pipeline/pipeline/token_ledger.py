"""
Persistent token usage ledger. Ported from eis_pipeline/pipeline/token_ledger.py.

Tracks per-model usage across runs in a human-readable JSON file. Recomputes
lifetime totals on every write so they stay in sync. Concurrent writes are NOT
safe — assumes one pipeline run at a time.
"""
