...
Ported from eis_pipeline/pipeline/llm_client.py with model pricing extended
for claude-opus-4-7 (retries-only per build brief §9).

Features:
- dry-run mode (no API calls; estimates tokens)
- per-model usage tally
- hard budget cap (raises BudgetExceededError)
