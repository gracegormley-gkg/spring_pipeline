...
Loads a doc from docs_with_digits.json (the v1 backup text source; per-page
OCR confidence is not available in this format), normalizes text (whitespace-
preserving Unicode quote folding + soft-hyphen dehyphenation), builds an
alignment map from normalized→raw character offsets, and splits raw text into
~CHARS_PER_FAKE_PAGE-sized fake pages on line boundaries.

Per synthesis_plan.md:
  - Conservative cleanup ONLY (whitespace, soft-hyphen dehyphenation, Unicode
    quote norm). No `0↔O` or `1↔l`. Verbatim substring verification post-
    normalization depends on a tight 1:1 alignment for unmodified spans.
  - OCR confidence is metadata, never a filter. This ingest path reads from
    docs_with_digits.json which has no per-page confidence; the
    "OCR confidence loosened" policy (§Key design decision 6) attaches when
    a future S3/OCR-JSON ingest path is added.

Output schema (in-memory IngestArtifact dataclass; not persisted as Pydantic):
  publication_id      : doc_key
  physical_record_ids : [doc_key]   (Stage 1.5 may extend if grouping merges)
  raw_text            : str          (untouched; basis for char_offset_raw)
  text_normalized     : str          (post-normalization; for matching/search)
  alignment_map       : list[AlignmentSpan]
  pages               : list[FakePage]
  nul_metadata        : dict         (raw NUL response if found)
  raw_text_hash       : "sha256:<hex>"

Ported verbatim from v1_multiagent_pipeline/pipeline/ingest.py.
"""
...
class IngestArtifact:
    ...
    def to_json(self) -> dict[str, Any]:
        """JSON view excludes raw_text + text_normalized — too big for casual reads.
        Use raw_text directly from the in-memory artifact for downstream stages."""
        return {
