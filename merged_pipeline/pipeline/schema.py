"""Locked Pydantic v2 schema for the merged EIS pipeline.

Base: v1_multiagent_pipeline/pipeline/schema.py (forward-compatible v2 shape).
Additions per synthesis_plan.md:
  - CriticVerdict + verdicts list in ValidationField (tiered critic, ultraplan.md)
  - Extended ProvenanceSource values (fallback paths, NER sources, gap-fill, backup)
  - SpanRecord support inside CommentBlock (for span-ID quote selection)
  - layman_summary parent linkage (status conventions)
...
Status conventions:
  ...
  - "skipped_budget_cap"           : producer didn't run because budget was already exhausted
  - "extracted_from_mets"          : sourced from NUL/METS
  - "extracted_from_cover"         : sourced from cover-page text
  - "fallback_keyword_search"      : section missing; field used full-text keyword search
  - "fallback_first_n_chunks"      : section missing; field used first-N-chunks heuristic
...
ProvenanceSource = Literal[
    ...
    "nul_api",
    "mets_xml",
    "docs_with_digits_json",  # backup text source; page=None
    # Deterministic extraction
    "regex",
    "fuzzy_match",
    "controlled_vocab_match",
    # ML / embedding
    "embedding_fallback",
    "gliner",
    # NER layers (PIPELINE_OVERVIEW.md Stage 0)
    "spacy_person",
    "spacy_org",
    "dict_agency",
    "dict_tribe",
    ...
    "sonnet_extractor",
    "opus_extractor",
    # Retrieval fallback markers (synthesis_plan.md §Key design decision 0)
    "fallback_keyword_search",
    "fallback_first_n_chunks",
]
...
CriticTier = Literal["haiku", "sonnet", "opus", "skipped_budget_cap"]
CriticVerdictValue = Literal["pass", "partial", "no", "skipped"]
...
class Provenance(BaseModel):
    ...
    source_text_hash: str | None = None  # sha256 of the source span; null for nul_api

    # Document-relative pointers (null when source has no doc location, e.g. nul_api)
    page: int | None = None
...
class Quote(BaseModel):
    ...
    # Span-ID linkage (synthesis_plan.md §Stakeholders — quote selection):
    # the span_id chosen by the LLM that backs this quote. Code copied text_raw
    # from the alignment map keyed by this span_id; the model never typed the text.
...
class SectionRecord(BaseModel):
    ...
    detection_method: Literal[
        "regex", "ai_toc", "embedding_fallback", "default_pages", "manual"
    ] = "regex"
...
# ---------------------------------------------------------------------------
# Comment blocks + spans (Stage 3f)
#
# SpanRecord supports span-ID quote selection per synthesis_plan.md:
# Stage 3f sentence-splits each comment block's comment_text, assigns each
# span a stable span_id, and lets the LLM pick a span_id (not type text).
# ---------------------------------------------------------------------------
...
class SpanRecord(BaseModel):
    ...
    page: int
    is_comment_text: bool = True  # False if span belongs to agency response

    @model_validator(mode="after")
    def _validate_offsets(self) -> "SpanRecord":
        s, e = self.char_span_raw
        if s < 0 or e <= s:
            raise ValueError(
                f"SpanRecord {self.span_id} char_span_raw invalid: ({s}, {e})"
            )
        return self
...
class CommentBlock(BaseModel):
    ...
    # Comment/response split (Stage 3f). Either may be None if split failed.
    comment_text_span: tuple[int, int] | None = None
    agency_response_span: tuple[int, int] | None = None
    split_status: Literal["ok", "comment_response_split_failed", "no_response"] = "ok"

    # Span-level granularity for quote selection.
...
class Stakeholder(BaseModel):
    ...
    appearance_order: int | None = None  # 1..N by first-mention order in doc


# ---------------------------------------------------------------------------
# Validation — tiered critic (ultraplan.md)
# ---------------------------------------------------------------------------

class CriticVerdict(BaseModel):
    model_config = ConfigDict(extra="forbid")

    field_path: str               # e.g. "summary.value", "alternatives[2].description"
    tier: CriticTier
...
class EISRecord(BaseModel):
    ...
    @model_validator(mode="after")
    def _stance_quote_span_belongs_to_block(self) -> "EISRecord":
        """If a stance quote was selected via span_id, that span must belong to the
        stakeholder's comment_block AND must be is_comment_text=True (never an
        agency-response span). Synthesis_plan.md §Stakeholders quote selection."""
        blocks_by_id = {b.block_id: b for b in self.comment_blocks}
        for sh in self.stakeholders:
            block = blocks_by_id.get(sh.comment_block_id)
            if block is None:
                continue
            spans_by_id = {sp.span_id: sp for sp in block.spans}
            for sr in sh.stance_records:
                if sr.quote is None or sr.quote.span_id is None:
                    continue
                span = spans_by_id.get(sr.quote.span_id)
                if span is None:
                    raise ValueError(
                        f"stakeholder {sh.comment_author.name!r}: quote.span_id="
                        f"{sr.quote.span_id!r} not in block {block.block_id!r} spans"
                    )
                if not span.is_comment_text:
                    raise ValueError(
                        f"stakeholder {sh.comment_author.name!r}: quote span "
                        f"{sr.quote.span_id!r} is an agency-response span — "
                        f"cannot attribute as stakeholder stance"
                    )
        return self
