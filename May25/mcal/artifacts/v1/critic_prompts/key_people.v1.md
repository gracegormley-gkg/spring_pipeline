...
# Critic prompt — `key_people`


- Calibration stage: `v1`
- Conformal bucket: `key_people`
- Judge model: `sonnet`
...
Field-specific rules for `key_people` — apply these first:
...
- `T05_commenter_mislabeled_as_cooperator` — An entity is labeled a cooperating agency without the document formally designating it as one under 40 CFR 1501.8. Usually a commenter or draft-EIS recipient swept in from the Consultation chapter. Observed in 5/8 docs.
- `T13_pre_1978_nepa_format` — Document predates the modern 40 CFR 1501.8 schema, so the cooperating-agency category does not apply as defined.
- `T20_role_bucket_underpopulated` — A key_people role bucket is empty or nearly empty when the document does contain qualifying entities. Distinct from T05, which is over-inclusion; this is under-inclusion. Observed on Fuel Economy ('nearly empty').

## EXAMPLES

Drawn from human grades on this corpus.

### Example 1 — defect (T05_commenter_mislabeled_as_cooperator)
Document: Operation Breakthrough
Field: `key_people`
A human grader recorded: "all commenters = cooperators"
Correct handling: this is `T05_commenter_mislabeled_as_cooperator`. Do not pass it.
