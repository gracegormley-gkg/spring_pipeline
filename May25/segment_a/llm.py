...
# --- Usage / cost tracking --------------------------------------------------
# Process-wide collector: while a "usage session" is active, every successful
# call_with_usage() appends its normalized usage dict to a list. This is
# coarser than thread-local tracking but lets us cover m2.py's
# ThreadPoolExecutor map-reduce without changing call signatures everywhere.
# Sessions are intended to be opened once per doc in run.py:process_doc.

_usage_lock = threading.Lock()
_usage_collector: Optional[list[dict]] = None

# Default Bedrock prices ($ per 1M tokens). Same defaults as
# people_pipeline/settings.py and statements_pipeline/settings.py — verify
# against the AWS Bedrock invoice for billing.
PRICES_USD_PER_M = {
...
def start_usage_session() -> None:
    """Begin recording per-call usages. Call before a logical unit of work."""
    global _usage_collector
    with _usage_lock:
        _usage_collector = []


def end_usage_session() -> list[dict]:
    """Stop recording; return the list of usage dicts captured since start."""
    global _usage_collector
    with _usage_lock:
        out = _usage_collector or []
...
def _record_usage(usage: dict) -> None:
    with _usage_lock:
        if _usage_collector is not None:
...
def price_for_model(model: str) -> dict:
    ...
    First matching key in PRICES_USD_PER_M wins, so order specific keys before
    generic ones.
    """
    m = (model or "").lower()
